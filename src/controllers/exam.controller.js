...
exports.list = asyncHandler(async (req, res) => {
  ...
  const payload = req.user.role === 'student' || req.user.role === 'parent'
...
exports.generate = asyncHandler(async (req, res) => {
  ...
  if (!title || !subject || !chapter || !classRoom || !academicYear) {
    return res.status(400).json({ message: 'title, subject, chapter, classRoom, and academicYear are required' });
  }

  const allowedClassIds = await teacherClassIds(req);
  if (allowedClassIds && !allowedClassIds.map(String).includes(String(classRoom))) {
    return res.status(403).json({ message: 'Teacher can create exams only for assigned classes' });
  }
  ...
  const exam = await Exam.create({
    ...
    classRoom,
    academicYear,
    createdBy: req.user.role === 'teacher' ? req.user.teacher : undefined,
    difficulty,
...
exports.update = asyncHandler(async (req, res) => {
  ...
  if (allowedClassIds && !allowedClassIds.map(String).includes(String(exam.classRoom))) {
    return res.status(403).json({ message: 'You cannot edit this exam' });
  }
...
exports.publish = asyncHandler(async (req, res) => {
  const exam = await Exam.findById(req.params.id);
  if (!exam) return res.status(404).json({ message: 'Exam not found' });
  if (!exam.questions.length) return res.status(400).json({ message: 'Cannot publish an exam without questions' });
...
exports.startAttempt = asyncHandler(async (req, res) => {
  if (req.user.role !== 'student') {
    return res.status(403).json({ message: 'Only students can attempt exams' });
  }

  const exam = await Exam.findById(req.params.id);
  if (!exam || exam.status !== 'published') {
    return res.status(400).json({ message: 'Exam is not available for attempt' });
  }
...
exports.submitAttempt = asyncHandler(async (req, res) => {
  ...
  submission.percentage = exam.totalMarks ? Math.round((score / exam.totalMarks) * 100) : 0;
...
exports.results = asyncHandler(async (req, res) => {
  ...
  } else if (req.user.role === 'parent' && req.user.linkedStudent) {
...
exports.classReport = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    summary: {
      attempted,
      averageScore,
      passRate: attempted ? Math.round((passCount / attempted) * 100) : 0,
...
exports.deleteExam = asyncHandler(async (req, res) => {
  ...
  if (exam.status === 'published') {
    return res.status(400).json({ message: 'Close the exam before deleting' });
  }
